...
who_regions_points <- data.frame(
  Region = c("Africa", "Americas", "Europe", "Eastern Mediterranean", "South-East Asia", "Western Pacific"),
  lon = c(20, -60, 20, 45, 90, 130),
  lat = c(0, 10, 50, 25, 20, 0),
  VaccinationRate = c(72, 89, 94, 85, 91, 88)
)  

who_regions_sf <- st_as_sf(who_regions_points, coords = c("lon", "lat"), crs = 4326)

library(ggplot2)
ggplot(who_regions_sf) +
  geom_sf(aes(color = Region), size = 5) +
  theme_minimal() +
  labs(title = "WHO Regions (punten)")



p <- ggplot() +
  geom_sf(data = world, fill = "gray90", color = "white") +
  geom_sf(data = who_regions_sf, aes(fill = VaccinationRate), size = 5, shape = 21, color = "black") +
  theme_void() +
  scale_fill_viridis_c(
    trans = "log", breaks = c(50, 60, 70, 80, 90),
  ...
  labs(
    ...
    subtitle = "Vaccination grade per WHO region"
  ) +
  theme(
...
#map per WHO regio de vaccinatiegraad laten zien.
```
