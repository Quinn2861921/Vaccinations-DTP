...
cleaned_healthspending_data$Continent <- clean_names(cleaned_healthspending_data$Continent)
cleaned_dtpdata$Continent <- clean_names(cleaned_dtpdata$Continent)
childdeath_with_period$Continent <- clean_names(childdeath_with_period$Continent)
...
# 🔄 Zet om naar lang formaat voor plotten
...
# 🖼️ Maak barplot
ggplot(plot_data, aes(x = Group, y = Value_scaled, fill = Metric)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = round(Value, 1)),
            position = position_dodge(width = 0.9), vjust = -0.4, size = 4) +
  labs(
    title = "Gemiddelden van Top & Bottom 3 WHO-regio’s (2022)",
    subtitle = "Health spending per capita, DTP3 coverage en kindersterfte",
    x = NULL,
