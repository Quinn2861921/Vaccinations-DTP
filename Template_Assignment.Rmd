...
ggplot(merged_healthdata_df, aes(x = Period, y = DollarPerCoveragePoint, group = Region, color = Region)) +
  ...
  geom_point(size = 2) +     
  labs(
    title = "Dollar per Coverage Point over Tijd",
    x = "Periode",
...
ggplot(merged_healthdata_df, aes(x = Period, y = DeathsPerSpending, group = Region, color = Region)) +
  ...
  geom_point(size = 2) +
  labs(
    title = "Child Deaths per Spending over Tijd",
    x = "Periode",
...
# Filter op jaar 2022 en bereken gemiddelden per 'Continent' (Region)
# Filter op jaar 2022 en bereken gemiddelden per continent
...
# Combineer alle data op Continent
combined <- spending_2022 %>%
  inner_join(coverage_2022, by = "Continent") %>%
  inner_join(childdeath_2022, by = "Continent")

# Stap 4: Selecteer bottom 3 en top 3 continenten zonder overlap
combined_sorted <- combined %>% arrange(Spending)

bottom3 <- combined_sorted %>% slice(1:3)

top3 <- combined_sorted %>%
  filter(!(Continent %in% bottom3$Continent)) %>%
  slice_tail(n = 3)

selected <- bind_rows(top3, bottom3)

# Zet data lang voor ggplot
plot_data <- selected %>%
  pivot_longer(cols = c(Spending, AvgCoverage, AvgChildDeath),
               names_to = "Metric",
               values_to = "Value")

# Factorlevels voor nette volgorde in plot
plot_data$Continent <- factor(plot_data$Continent,
                             levels = unique(selected$Continent))

# Optioneel: schaal de waarden per Metric tussen 0 en 1
plot_data <- plot_data %>%
  group_by(Metric) %>%
  mutate(Value_scaled = (Value - min(Value)) / (max(Value) - min(Value))) %>%
  ungroup()

# Plot grouped barplot met geschaalde waarden
ggplot(plot_data, aes(x = Continent, y = Value_scaled, fill = Metric)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = round(Value, 2)),
            position = position_dodge(width = 0.9), vjust = -0.5, size = 3) +
  labs(
    title = "Vergelijking per continent: Gezondheidsuitgaven, Vaccinatiegraad & Kindersterfte (2022) (geschaald)",
    x = "Continent / Regio",
    y = "Geschaalde waarde (0-1)"
  ) +
  scale_fill_manual(values = c(
    Spending = "#1f77b4",
    AvgCoverage = "#2ca02c",
    AvgChildDeath = "#d62728"
  )) +
  theme_minimal()
```
